...
The release objective includes all four requested names: `platformio-mcp`, `pio-mcp`, `platformio.mcp`, and `pio-agent`. Evaluate each on every suitable distribution channel. A canonical package alone does not satisfy alias coverage where functional aliases are permitted and project control is verified.
...
| GHCR | Publish all four candidate image names under the verified project owner, pointing to the same multi-platform image digest. |
