...
The CLI command `pio-agent flash-verify --project-dir <dir> [--environment <env>] [--upload-port <port>] [--monitor-port <port>] [--baud <rate>]` uses this same workflow. Verification flags are `--expect`, `--fail-on`, `--timeout`, `--settle`, `--stability-window`, `--max-lines` and `--stop-open-sessions`. Scoped grants are `--workflow-approval-id`, `--approval-id`, `--config-approval-id`, `--selection-approval-id`, `--monitor-approval-id`, `--read-approval-id`, `--preflight-discovery-approval-id`, `--discovery-approval-id`, `--decode-approval-id` and `--decode-config-approval-id`. `--json` prints the shared report; failed or inconclusive verification exits unsuccessfully. The existing `flash` and `agent-flash-monitor-verify` commands remain available.
...
Results explicitly report `firmware_identity: identity_unverified` until the upload manifest is bound to immutable images and ELF. Automatic crash decoding currently uses the selected environment ELF and reports that matching the uploaded firmware has not been verified. The public workflow now reserves the monitor endpoint and discovered USB identity before upload, retains those leases through uploader closure, and then opens the monitor without releasing them. The same-port uploader uses that retained custody; separate upload ports keep their own process custody while the monitor reservation remains held. For same-port uploads, cancellation requests uploader termination, and uncertain process cleanup keeps the reservation pending. Startup discovery uses five explicitly scoped snapshots for this composition. Matching-image evidence and physical acceptance remain outstanding. Registration does not establish complete PAR-12 acceptance.


## OTA upload

Opt-in `pio_upload_ota` accepts the reference `host`, `project_dir`, `env`, `port`, `auth`, `filesystem`, `build`, `timeout_s` and `verify_reachable` fields. ESP32/ESP8266 port defaults come from computed upload flags or 3232/8266 respectively. Build mode builds the image without uploading, freezes its bytes, and calls the registered framework uploader directly. `build=false` transfers an existing image. Custom artifacts can use `image_path` and `expected_image_sha256`. Firmware uploads may also specify `elf_path` to match an explicit workspace ELF against the retained ESP-IDF application image’s embedded SHA-256. A mismatch, missing embedded identity, or incompatible ELF stops the upload; filesystem uploads do not accept this option. Matching ELF bytes are archived for later decoding, and the report includes `elf`, `elf_correspondence: embedded_hash_match`, and `firmware_archive_path`. Without explicit matching, correspondence remains `identity_unverified`. Embedded hashes are image metadata, not signatures or proof that the device executed the image.
...
`power_profile` is available in normal mode; `pio_power_profile` is its opt-in reference alias. Both default to `source: "serial"` and support the reference duration, port, baud, pattern, voltage, threshold, bucket and owned-trigger arguments. The serial path closes its one-shot meter session and retains the trigger monitor. PPK2 requests require `source: "ppk2"`, explicit `mode: "ampere" | "source"`, `dut_port`, `voltage_mv` and `current_limit_ma`, with the configured isolated host environment. PPK2 source current trips are limited to 600 mA and ampere trips to 1000 mA; these are software thresholds, not hardware regulation. Omitting `port` selects exactly one host-enumerated USB endpoint with Nordic PPK2 VID/PID `1915:c00a`; zero or multiple candidates require explicit selection. Explicit multi-interface selection retains the complete device exclusion and interface topology checks. PPK2 accepts `trigger` with `trigger_session_id` for a live monitor owned by the same connection and project. The monitor must hold exactly the discovered DUT resource set; matching text alone does not establish device identity.
...
Automatic PPK2 selection uses USB descriptors as a discovery hint, not proof of model or wiring. Nordic's [device enumeration example](https://devzone.nordicsemi.com/f/nordic-q-a/108586/ppk2-via-python-in-linux-of-chromebook/470432) identifies `1915:c00a`. The pinned ppk2-api 0.9.2 selector uses product/description strings and likewise requires the caller to resolve multiple candidates. Descriptor selection does not guess the protocol interface on devices exposing two matching ports; pass `port` for those devices. Physical acceptance remains required.

### CLI power profiling

`pio-agent power-profile --project-dir <dir> --port <meter> --seconds 10 --json` uses the canonical serial power profiler. PPK2 uses `--source ppk2 --mode ampere|source --dut-port <port> --voltage-mv <mV> --current-limit-ma <mA>`; omit `--port` for unambiguous automatic meter selection. `--baud`, `--pattern`, `--provenance`, `--buckets`, `--sleep-threshold-ma` and `--max-lines` map to the corresponding serial options. Source-specific validation rejects irrelevant or invalid options before discovery.

Interactive approvals remain within the invocation; `--approve` explicitly approves each required scoped request without overriding denials. JSON mode does not prompt without `--approve`. Separate `--profile-approval-id`, `--approval-id`, `--config-approval-id`, `--selection-approval-id`, `--discovery-approval-id`, `--read-approval-id`, `--host-approval-id` and `--power-approval-id` options are accepted only by their applicable source. Both clients are closed on success or failure, and cleanup errors result in unsuccessful exit. Owned trigger-session workflows remain available through the persistent MCP connection; this one-shot CLI command does not borrow sessions from another connection.

OTA reports include parsed build `memory` and source-located `errors`; failed-build responses retain timing, the log reference and bounded output. A failed build marker blocks transfer even if a process reported zero exit status. Successful transfer still reports `runtime_verified: false`. The uploader output tail is bounded to forty lines and 16,000 characters. ICMP reachability is still reported as unprobed rather than inferred from transfer success.

`upload_ota` is available in normal MCP mode. Compatibility mode additionally exposes `pio_upload_ota`; both accept the same documented OTA fields and use the same underlying approvals, immutable-image transfer and result reporting. Denying either name continues to constrain both firmware and filesystem transfers. Normal users do not need compatibility mode to access OTA.

Normal MCP mode exposes `flash_verification` with the same parameters and retained-firmware approval/resume controls as opt-in `pio_flash_and_verify`. The legacy `agent_flash_monitor_verify` tool is retained. Both new entrypoint names enforce the same workflow and sub-operation permissions; a resume ID remains connection-local and does not grant upload permission.
