...
Results explicitly report `firmware_identity: identity_unverified` until the upload manifest is bound to immutable images and ELF. Automatic crash decoding currently uses the selected environment ELF and reports that matching the uploaded firmware has not been verified. The public workflow now reserves the monitor endpoint and discovered USB identity before upload, retains those leases through uploader closure, and then opens the monitor without releasing them. The same-port uploader uses that retained custody; separate upload ports keep their own process custody while the monitor reservation remains held. For same-port uploads, cancellation requests uploader termination, and uncertain process cleanup keeps the reservation pending. Startup discovery uses five explicitly scoped snapshots for this composition. Matching-image evidence, dedicated CLI exposure and physical acceptance remain outstanding. Registration does not establish complete PAR-12 acceptance.
