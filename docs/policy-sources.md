...
Run `pio-agent dashboard --operator` locally to open and enroll an operator browser session. That session approves or denies each pending request with one click; no authorization code, copied token, or second prompt is required. Enrollment uses a single-use launch ticket and an HttpOnly session cookie, expires after eight hours, and is lost when the server restarts. Ordinary MCP dashboard launch tickets remain view/operation-only and cannot enroll an operator. `PIO_MCP_APPROVAL_TOKEN` remains an optional 32–256-character host-configured capability for non-browser operators; the browser UI never requests or stores it. The local `pio-agent approve <id>` and `pio-agent deny <id>` commands remain available.
