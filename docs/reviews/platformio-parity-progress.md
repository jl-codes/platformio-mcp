...
1. **Acceptance integration � Codex:** apply the explicit maintainer 3.1.0 decision recorded in platformio-acceptance-requirements.json. The five named certifications are deferred, not passed. Preserve completed ESP32 evidence and validate all retained correctness, authorization, ownership/cleanup and credential-protection assertions. Assemble the acceptance packet with accurate historical applicability; no new hardware run is needed solely for the amendment.
2. **Artifact and installation gates � Codex:** preserve native wheel/container runtime and package integrity checks; only minimum-OS and container device-passthrough certifications are deferred. Run the existing mandatory release checks on the delivery snapshot and verify installation of the actual published artifacts.
