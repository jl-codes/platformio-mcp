...
1. **Acceptance packet and artifact gates — Codex:** retained software/hardware evidence and maintainer deferrals are integrated. Candidate 36187004212 at 8526db76 passed all five native wheel installations, both native containers, release gates, exact npm install/upgrade and container archive binding. The next candidate, 36188364150 at 746f26cb, assembled and strictly validated the acceptance packet after the mixed-line-ending correction; all five wheels, both containers and artifact gates passed. Uploading the packet failed because the action excludes hidden output directories. The workflow now writes this validated packet to a dedicated visible directory. Complete its artifact retention on the final snapshot. Preserve historical evidence applicability and the unchanged same-commit external acceptance gate.
2. **npm trusted publishers — Codex/user MFA:** all ten grants are created for jl-codes/platformio-mcp, release.yml, including pio-agent and seven scoped aliases. PyPI grants and ten GHCR targets remain configured; they are independent of npm MFA.
3. **Merge/publication — Codex:** PR #26 is ready but unmerged; 3.1.0 is unpublished. After retained gates pass, merge and publish eligible channels through existing controls, then verify actual registry and installed artifact identity. All seven namespace families and the four recorded npm naming-rule rejections remain unchanged.
