...
1. **Implementation / evidence integration � Codex:** debugger, policy and operator-session decisions are resolved; no new product defect remains from the completed connected-board acceptance. Commit 8f612ce3 contains the protected per-project PyPI workflow, ten GHCR targets and OTA fixture. Required CI 36163918510 and installer 36163918492 passed. The 40-tool reconciliation below identifies behavioral evidence and its limits; registration/CI alone is not full acceptance. Next: assemble the original final-revision acceptance packet from applicable evidence once outstanding original physical/host gates can be satisfied; do not relabel historical tests as newly executed.
2. **Original physical / host acceptance � user/lab and host operators supply resources; Codex executes:** ESP32-S3 native debugger, serial/crash/recovery, flash/verify, partition/core-dump, authenticated firmware/filesystem OTA, runtime/filesystem verification, native-fixture restoration and legacy USB filesystem upload have concrete evidence. Remaining physical gates are PAR-HW-02 (Cortex-M fault PC/LR) and the ESP8266 and PPK2 parts of PAR-HW-07. Minimum-OS, actual host permission/enforcement and container non-root device-access acceptance remain under NS-HOST-* / NS-CONTAINER. Existing configured gl-mac at 192.168.1.142:22 timed out; configured Tailscale Mac/Linux peers report offline. Local Docker was started but its backend terminated during inference-socket initialization; no container test was claimed passed. Exact resources/actions are below.
3. **Publication � Codex:** all three approved PyPI pending publishers/protected environments and marker are verified; all ten approved GHCR targets/marker are configured. Existing npm credentials, seven scoped 3.0.0 publications and four unscoped naming-rule rejections remain valid. No further grant or naming retry is needed. Next: publish through existing protected workflows and verify registry/installed identities after the original aggregate acceptance and merge gates pass. Pending publishers are not published/reserved packages.
4. **Delivery � Codex:** PR #26 remains unmerged; 3.1.0 remains unpublished. Existing merge/tag/publication authorization is sufficient. Missing original acceptance is the delivery blocker; no new requirements or reduced milestone replace the full release.

### Exact outstanding equipment and hosts

Owner: user/lab operator supplies or grants access; Codex runs the existing acceptance. These are original requirements, not additional scope.
