...
1. **Acceptance packet and artifact gates — Codex:** retained software/hardware evidence and maintainer deferrals are integrated. Candidate 36187004212 at 8526db76 passed all five native wheel installations, both native containers, release gates, exact npm install/upgrade and container archive binding. Its acceptance packet stopped on retained plugin metadata hashes collected on mixed Windows/Unix line endings. The producer now accepts the historical byte hash under either line-ending convention while requiring exact unchanged Git content; focused checks of all three plugin metadata files passed. Complete strict packet assembly on the final snapshot. Preserve historical evidence applicability and the unchanged same-commit external acceptance gate.
