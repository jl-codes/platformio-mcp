...
1. **Acceptance packet and artifact gates — Codex:** retained software/hardware evidence and maintainer deferrals are integrated. Candidate 36186067287 at c0248337 passed all five native wheel installations and both native container MCP/policy/EOF checks. Artifact assembly stopped at aliases:check because the previously rejected flashagent wrapper retains a packagePath. The synchronizer now validates that prepared blocked wrappers remain publication-disabled and checks their versions; focused aliases:check passed. Complete the remaining mandatory artifact checks and strict acceptance packet assembly on the final snapshot. Preserve historical evidence applicability and the unchanged same-commit external acceptance gate.
2. **npm trusted publishers — Codex/user MFA:** canonical platformio-mcp and pio-mcp grants are created for jl-codes/platformio-mcp, release.yml. The pio-agent grant is waiting for npm MFA, followed by the seven scoped aliases. PyPI grants and ten GHCR targets remain configured; they are independent of npm MFA.
