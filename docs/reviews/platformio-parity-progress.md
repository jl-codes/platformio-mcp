...
Latest goal read on 2026-09-25 confirms active. The user explicitly superseded the old objective's merge restriction; merge/publication authorization remains in force. No objective edit is claimed. Branch: `codex/platformio-parity-permissions`.
...
1. **Acceptance integration � Codex:** apply the explicit maintainer 3.1.0 decision recorded in platformio-acceptance-requirements.json. The five named certifications are deferred, not passed. Preserve completed ESP32 evidence and validate all retained correctness, authorization, ownership/cleanup and credential-protection assertions. Assemble the acceptance packet with accurate historical applicability; No certification reruns are required for the amendment. PAR-HW-03 legacy timestamp capture now passes on the identified ESP32-S3 after fixing forced raw mode and Windows headless-console exit; MCP stop confirmed inactive. Native quality harness correction, concrete test mappings and candidate packet producer are integrated in this batch; Latest native PlatformIO quality job passed at dd284779. Candidate exposed POSIX EOF output truncation; fix now drains pending replies after unchanged cleanup. Focused shutdown/lease checks passed 26 tests; a test-only child-release race waits for both record and gate removal. Evidence assembly combines actual required-host results for Windows-only cases. At 6d3e9bfc all five native wheel installations passed. Container MCP and deny-policy checks passed; its harness denied its own status query, so the fixture now permits only that read query. All five wheel hosts passed again at 06bca376. Container harness must await asynchronous read responses before EOF; this sequencing correction leaves server disconnect cleanup unchanged. Next candidate must complete containers, artifact checks and packet assembly. npm trust inspection requires fresh browser MFA (user action requested once); this is separate from PyPI/GHCR grants.
2. **Artifact and installation gates � Codex:** preserve native wheel/container runtime and package integrity checks; only minimum-OS and container device-passthrough certifications are deferred. All ten local npm distributions passed 3.0.0-to-3.1.0 installation/update/uninstall; isolated Codex plugin refresh/removal passed with exact inventory match. Run the existing mandatory candidate gates on the committed snapshot, then verify actual published artifacts. The original mandatory same-commit external acceptance gate is unchanged; the packet producer runs only for nonpublishing candidates.
3. **Merge/publication � Codex:** PR #26 remains unmerged and 3.1.0 unpublished. Existing PyPI grants, ten GHCR targets and npm authorization are configured. Merge after retained gates pass; publish independent eligible channels through protected workflows and verify registry identities. Preserve all seven namespace families and the four recorded npm naming-rule rejections.

### Deferred certification equipment and hosts

Owner: user/lab or host operator for later certification. The maintainer has deferred the physical/minimum-OS/device-passthrough portions listed here from 3.1.0 gates, not marked them passed. Ordinary native installation/runtime and security acceptance remain required.

| Original requirement | Concrete resource and next action |
| --- | --- |
| PAR-HW-02 | A Cortex-M board with a supported SWD/JTAG probe (for example, an STM32 development board with onboard ST-Link), data USB cable and accessible debug connection. Identify the board/probe so the repository fault fixture can capture PC/LR/fault registers against its flashed ELF. The Xtensa ESP32-S3 cannot establish this result. |
| PAR-HW-07, ESP8266 | An ESP8266 development board with USB serial and sufficient OTA application/filesystem space, connected to an accessible 2.4 GHz network. Identify its USB identity before fixture upload and bind its network endpoint from serial. |
| PAR-HW-07, power | Nordic PPK2, its data USB connection, DUT and measurement wiring with a known current path/common ground. Source-mode acceptance additionally needs explicit DUT voltage/current limits and isolation from competing power sources; none are inferred from USB connection. |
| NS-HOST-win32-x64 | Windows 10 x64 host/VM with Python 3.11+ and no global Node for the minimum-OS installed wheel checks; reuse the existing unchanged-launcher evidence where applicable. |
| NS-HOST-darwin-arm64 / darwin-x64 | Apple Silicon and Intel Mac hosts/VMs at the selected wheel minimum macOS 14.0 for native wheel/serial ABI and lifecycle checks. The launcher explicitly labels 13.5 as the bundled Node floor; the existing macosx_14_0 wheel tags impose the stricter install floor. These are distinct runtime/package constraints, not a new requirement to retag or expand support. Bring the already configured gl-mac online first; its architecture/OS cannot be inferred from a timed-out connection. |
| NS-HOST-linux-arm64 / linux-x64; NS-CONTAINER | Native ARM64 and x64 Linux hosts with glibc 2.28, kernel 4.18 and GLIBCXX_3.4.25 minimum compatibility, Python 3.11+, and container support/device passthrough for existing non-root device-access acceptance. Bring configured Linux peer spark-1b95 online if it is an authorized suitable host. Local Docker Desktop currently fails at AppData/Local/Docker/run/dockerInference (file inaccessible / invalid listener path); restore that existing engine before local container tests. Current hosted native-install/container evidence remains reusable for unchanged components, but is not minimum-version or physical-device evidence. |
...
### 2026-09-20: Remote debugger endpoint and custody primitives

Added inert numeric IPv4/IPv6 endpoint parsing with canonical IPv6 and IPv4-mapped identity, bounded ports, and rejection of unresolved DNS, unspecified/multicast addresses, zone identifiers and pipe syntax. Remote endpoint custody reuses the existing lease lifecycle and requires a host-supplied target-custody capability; it never probes or claims ownership of an externally managed server. Equivalent addresses conflict, local loopback identities match owned backends, and failed target release retains endpoint exclusion for retry. Twenty-one focused parser/custody tests and TypeScript passed; plugin rebuilt. No remote network/hardware interaction occurred. Public remote startup and trusted target-binding configuration remain unwired; no completed remote parity is claimed.

### 2026-09-20: Host-bound remote startup composition

Added remote prepared startup using the shared supervised GDB path, retained ELF/init artifacts, canonical permission gates and remote endpoint custody. It rejects local-backend conflicts and mismatched host endpoint/identity, defers host revalidation/acquisition until authorized handoff, and retains retryable target cleanup. DebugCompatibilityClient now accepts a trusted embedding-host resolver (never a tool argument), routes server=null configurations through it, and fails explicitly when no binding is supplied. Nineteen focused startup/client tests initially passed; an additional client route/forgery regression was then added and all five client tests passed. TypeScript passed and plugin rebuilt. Standalone operator binding provisioning is still missing; remote physical acceptance, DNS/pipe and remaining backend support are not completed.

### 2026-09-25: carry explicit user consent through the operator approval path

Replaced the PIO manager skill's blanket prohibition on forwarding approvals with an exact-operation consent rule. Explicit conversational consent may be carried through the existing operator CLI, subject to host execution controls; MCP does not gain an approval endpoint. Denial, expiry, one-use scope, target and artifact checks remain enforced. Synchronized the shipped skill and checked generated-content consistency; no runtime code or policy defaults changed.

Applied the user's explicit COM5 fixture-upload consent through that path. Added the repository-owned hil-esp32s3 UART health/echo/command-triggered abort fixture. MCP build f787b330-e7d1-4a2b-836a-39b5c75ee51f passed; current-source MCP upload af8cd862-a32c-4e9e-951b-256c729e4f4d succeeded with esptool confirming ESP32-S3 revision v0.2 and verified flash hashes. Upload log: tests/fixtures/hardware/esp32s3/.pio-mcp-workspace/logs/upload/upload-2026-09-25T12-10-32-942Z-e75df49d.log. The first five-second 115200-baud health capture returned garbled bytes and no PIO_HIL_HEALTHY marker. Follow-up get_monitor_status confirmed inactive, so no monitor was left holding COM5. No crash command was sent. Physical acceptance is NOT passed; next hardware action is diagnose UART/boot output and complete the planned checks. The normal fixture remains flashed. No full suite, artifact build, or unrelated smoke test was rerun.

### 2026-09-25: standalone remote binding and monitor project configuration

Fixed PlatformIO monitor spawning to pass the selected project directory as cwd. Previously --environment could resolve outside that project and use the wrong serial settings. One five-second MCP capture against the unchanged, already-flashed COM5 fixture now returns four PIO_HIL_HEALTHY lines, no rejected patterns, and an inactive monitor after cleanup. This resolves the observed garbled capture; no reflash or firmware rebuild occurred. Full captured response is retained locally at .platformio-mcp/esp32s3-health-result.json. This verifies baseline UART health, not crash/echo/OTA/debug acceptance.

Standalone MCP and CLI debugger clients now default to operator-owned debug-targets.json mappings for numeric external endpoints, preserving embedding-host injection. The map must be bounded, strict, outside the project and match one project/environment/endpoint. Stable target IDs share per-account leases across alternate endpoints, mapping content participates in approval identity and is revalidated before acquisition/handoff, and cleanup retains normal lease semantics. Local USB selectors are rejected on remote startup rather than ignored. External-server ownership and remote-host-wide exclusion are not claimed. DNS, pipes and st-util remain open.

Validation: 13 focused mapping/startup/client checks and TypeScript initially passed. Added a direct default-resolver routing regression; all seven client checks passed. TypeScript passed again after that addition, and the checked-in plugin runtime/inventory was rebuilt once for the combined monitor and remote-binding changes. No generic audit, repeated full suite, or unrelated hardware run was performed.


## Operator-bound DNS debugger startup (2026-09-25)

Added canonical hostname matching against the operator map before any lookup, a three-second resolution bound, unicast address validation, and numeric pinning through GDB initialization and endpoint custody. The trusted binding retains the original hostname selection; mismatched names, ports and numeric bindings are rejected before startup. Mapping changes still invalidate target acquisition/handoff; no DNS name reaches GDB and a DNS change cannot redirect an existing preparation. Existing endpoint-specific approval gates remain intact.

Validation: 33 focused endpoint, operator-map, startup and client-routing tests passed, including unmatched-host lookup avoidance, timeout, invalid-address rejection, numeric pinning and source mismatch. TypeScript passed. This is software evidence; no remote debugger hardware session is claimed. Pipe/st-util integration, the original acceptance requirements and external release access remain in the blocker list above.


## Debugger result-contract parity (2026-09-25)

Compared the pinned reference debugger implementation against the current adapter, then fixed observed PAR-06 result gaps: separate console/log/target/unframed output, reference-shaped structured result payloads, command/session identity, elapsed duration, bounded last-200 MI records, numeric frame lines, frame arguments and breakpoint/exit/disposition fields. Ordered result_fields remain available without loss; arbitrary MI keys are projected only into null-prototype objects. Stream storage shares a 1 MiB/4096-chunk cap and record retention has an independent 1 MiB/200-record cap, with explicit truncation. Token correlation and process ownership are unchanged.

Validation: 38 focused transport, compatibility and process-owner tests passed; TypeScript passed. Transcript coverage verifies stream separation, duplicate result keys, inert __proto__ data, nested frame arguments, timeout state and output bounds. This closes these result gaps only; remaining debugger integration and original physical/release acceptance remain in the single blocker list above. Pinned reference inspected: src/platformio_mcp/debugger.py at a7b31021982f20b5406eaf80732899f8e75bd464.


## Debugger session metadata and retained script contents (2026-09-25)

Fixed the remaining observed session-result mismatches: init_script now returns the exact retained initialization contents, with the filename exposed separately as init_script_path. Start/list/stop share reference metadata including last_stop (the existing stopped field remains), observed stop_count, bounded records_buffered and transport error. Session history is limited to 4000 records and 1 MiB. Alternate trusted host implementations can report unavailable counters as null; no counts are fabricated. Successful stop reports confirmed process closure and released custody without claiming target-running verification.

Validation: three focused initialization/transport/session files passed (39 tests); the startup file initially failed one stale string-line expectation after the prior numeric-line parity fix. Updated that expectation to the pinned reference behavior; all seven startup tests then passed. TypeScript passed. Plugin runtime rebuilt for the changed capability. No hardware, full suite or distribution artifact matrix rerun. Backend integration, original acceptance and publisher requirements remain open above.


## Default OpenOCD startup reconciliation (2026-09-25)

Inspected PlatformIO Core v6.1.18 config/base.py, config/generic.py and process/server.py. Generic configuration supplies :3333; Core optionally converts an ordinary OpenOCD server into a GDB pipe command during launch. Our separately supervised TCP path implements that same default connection workflow and does not need to reproduce the internal pipe choice. Explicit arbitrary custom pipe commands are still unsupported; this does not claim otherwise.

Found and fixed the actual default-configuration failure: prepareLocalDebugBackend required a custom readiness expression even though Core permits none. Trusted OpenOCD now defaults to its exact selected-port GDB-listener message; explicit patterns remain validated, and other backends do not inherit an invented pattern. Probe binding, loopback-only listeners, owned-process supervision, grants and cleanup remain unchanged. Sixteen focused backend-selection/readiness/local-startup tests and TypeScript passed. Plugin rebuilt. Physical debugger acceptance and standalone st-util integration remain outstanding.

Sources: https://github.com/platformio/platformio-core/blob/v6.1.18/platformio/debug/process/server.py and https://github.com/platformio/platformio-core/blob/v6.1.18/platformio/debug/config/generic.py ; OpenOCD listener semantics: https://openocd.org/doc-release/html/Server-Configuration.html .


## Approved ESP32-S3 serial/crash acceptance (2026-09-25)

User explicitly approved the exact echo and CRASH serial commands on the already-flashed hil-esp32s3 fixture, COM5/CH340K location 1-5. Forwarded each one-use request through the operator CLI; no firmware build/upload, other target or peripheral operation occurred. Current-source MCP direct serial transport received PIO_HIL_HEALTHY and the exact echo. A second MCP process was denied with DEVICE_BUSY while the first owned COM5.

One CRASH command produced PIO_HIL_READY_BEFORE_CRASH, abort/backtrace, RTC_SW_CPU_RST, a new PIO_HIL_READY and subsequent PIO_HIL_HEALTHY. MCP decode_backtrace resolved the abort PC 0x42001759 to parity_fixture_crash() at fixture src/main.cpp:17 and the caller to loop():32. Existing ELF SHA-256 6073f2aa7277aaaa183b9bc466a80c8305cdc3ef2ab1ad02dfa953c8fb24d2ad matches the crash's reported prefix. Decoder reports matched_expected_elf; its flashedFirmwareVerified field remains false rather than overstating what decoding itself proves. Evaluating the complete captured output with the runtime assertion engine reports PanicTrace even though the earlier marker matched. This validates captured-output classification, not the entire live flash-and-verify stability workflow.
...
## Current-head CI failure correction (2026-09-25)

Inspected CI run 36138415941 for 43b04fc4 once its macOS job completed with failures. The job reported four failures: three plugin checks from an inventory referencing uncommitted dashboard asset index-CvYWrGjG.js, and the remote-startup fixture's symlinked macOS temporary directory. Recomputed the runtime inventory from all 36 committed runtime blobs (not mutable working web/dist), then validated a temporary snapshot of the committed plugin with requireRuntime=true. Validation passed with eight skills and runtime present. Unrelated dashboard/API edits remain untouched.

Canonicalized only the test fixture's mkdtemp result with realpathSync.native, preserving production rejection of symlinked lease roots. All four focused remote-startup tests passed locally; macOS confirmation belongs to the replacement CI run. No full local suite, native rebuild, firmware upload or hardware repetition was performed. Earlier in-progress jobs are superseded by this correction; do not wait on them. Backend scope clarification remains pending: the approved plan describes debugger/ST-Link functionality but does not explicitly name standalone st-util or arbitrary custom pipe commands. No scope waiver has been assumed.


## Windows installed-launcher console acceptance (2026-09-25)

Added the missing native Windows procedure to the existing Python host acceptance route. It initializes the installed launcher with stdin open, delivers CTRL_C_EVENT inside a hidden private console, and requires graceful exit within 15 seconds. The controller owns a kill-on-close Job Object before creating descendants; timeout/failure cannot leave the tested server running or signal the operator console. POSIX checks are unchanged.
...
## One-click operator session, native USB and core-dump acceptance (2026-09-25)

Integrated the other task's one-click UI/API changes with the user-selected operator-session separation. Local `pio-agent dashboard --operator` opens a single-use enrollment link; its HttpOnly session can approve/deny with one click and no code/token prompt. Ordinary MCP launch sessions, bearer tokens and forged same-origin click headers cannot gain approval authority. Non-browser operator capability and CLI approvals remain supported. Twenty focused API checks, one UI submission check, frontend build and TypeScript passed. Plugin API, UI assets and inventory were assembled from the same authorized working source snapshot; no unrelated changes were discarded.

Rediscovery first found the CH340K at location 1-5, then native USB at COM7 with serial 14:C1:9F:DC:20:4C. This matches the MAC recorded in the earlier fixture upload; the fresh target binding selected hil-esp32s3. Windows reports the native JTAG interface with WinUSB. MCP read the 4096-byte table at 0x8000 after both scoped grants: six partitions match the existing built table with no differences. Device table SHA-256 f4e3b6cfe370c81dca210d57f3c463f924a539842a19fff6eda4615278ee97a2; read log debug-2026-09-25T13-54-41-692Z-5d4d4004.log. The layout includes app0/app1, SPIFFS and a 64 KiB core-dump partition at 0x7f0000. This validates the current layout, not the still-required custom-layout acceptance.

Physical core-dump acquisition reused the previously approved crash; no additional crash or firmware upload occurred. Initial analysis attempts exposed absent analyzer configuration and missing explicit debugger trust roots; configured existing esp-coredump 1.10.0 and installed ESP32-S3 GDB, then verified executable selection before retrying. Final MCP acquisition/analysis passed: 65536 bytes from 0x7f0000, partition hash feb53ad7ad54a1d27877989736dd5e9e8d4c2d0e10f6144df1be875da041b0c5, converted core hash 33beef098e37e847aa6b9bbeb8c45199f2aede6621d8238896a565d89f943cf8. Firmware correspondence is prefix_matched at 64 bits, not full-hash proof. Six frames and 65 registers were decoded, including parity_fixture_crash at main.cpp:17 and loop at main.cpp:32. Final read log: debug-2026-09-25T14-06-57-561Z-87573a52.log. Full bounded MCP responses remain in .platformio-mcp/serial-acceptance-session.jsonl.

The real acquisition also exposed an analysis grant consumed before nested read approvals completed. Moved consumption to actual analyzer execution while retaining preflight, current-policy authorization and all read/host grants. Fifteen focused core-dump policy/export checks and TypeScript passed; the successful physical retry exercised the correction. Regenerated only the changed plugin runtime after this fix and validated its coherent inventory. No full suite, hardware upload, alias republication or optional audit was performed.


## Windows native debugger discovery and bounded failure diagnosis (2026-09-25)

Live debug_start on rediscovered native USB serial 14:C1:9F:DC:20:4C (COM7) exposed Core returning bin/openocd while the Windows package contains openocd.exe. Both registered-package discovery and final executable validation now try only an adjacent .exe when an extensionless Windows candidate is absent. Existing realpath/root/basename checks remain in force; no PATH, PATHEXT or shell fallback. Twenty-one focused discovery/backend-selection checks and TypeScript passed, including Windows registered/operator roots, script rejection and project-directed junction rejection.

The same-board no-load retry passed executable selection and exact host/target grants, but the owned backend exited before readiness (DEBUG_BACKEND_NOT_READY). Cleanup returned without a retained cleanup-pending session; physical debugger lifecycle is not passed. That error discarded the backend's already bounded output; it now carries outputTail into the existing redacted compatibility response. Seven focused readiness checks passed. The necessary diagnostic retry returned OpenOCD v0.11.0-esp32-20220706 output: LIBUSB_ERROR_NOT_SUPPORTED, LIBUSB_ERROR_NOT_FOUND, and esp_usb_jtag could not find or open device. Windows rediscovery reports MI_02 WinUSB/winusb.inf, problem code 0, while MI_00 remains usbser/COM7. Espressif documents driver installation for this error. Downloaded idf-env.exe from the official dl.espressif.com URL; Authenticode reports Valid, signer Espressif Systems (Shanghai) Co., Ltd. Started only driver install --espressif; it is waiting for Windows elevation (process session 93989), not claimed installed. No additional retry should run until its result or another relevant input changes. No firmware replacement, flash, core-dump reread or successful serial-test repetition occurred.

PyPI account settings visibly confirm primary email Verified and an active WebAuthn security device. Trusted-publisher setup remains pending the browser-required access-grant confirmation; the prepared form is not a configured publisher or name reservation. Existing npm publication and naming-rule dispositions remain unchanged.


The remaining custom-layout/native-serial acceptance now has a separate hil-esp32s3-native environment, inheriting the pinned original fixture, enabling USB CDC on the existing Serial/JTAG controller, and selecting partitions-hil.csv. The 8 MiB layout keeps NVS and the core-dump partition in place, gives each OTA app 0x320000 bytes, and moves SPIFFS to 0x650000. The original UART environment and source are unchanged. MCP build task 1ef8b4aa-2052-4361-89dd-9c3603915518 passed (18736 bytes RAM, 261145 bytes application flash); log build-2026-09-25T14-34-56-554Z-67850678.log. This is build evidence only: native fixture has not been uploaded, compared against the device, or used for live flash/verify acceptance. Nine compatibility-adapter checks and TypeScript also passed after the bounded startup-diagnostic correction. Final plugin runtime/inventory validation passed with 36 files and eight skills, reusing the unchanged dashboard assets.


### 2026-09-25 � Same-board native JTAG and acceptance fixes (working tree after 98c26cf3)

Bound native USB VID:PID 303A:1001, serial 14:C1:9F:DC:20:4C, currently COM7. Signed Espressif oem134.inf/WinUSB problem code 0 cleared the former driver blocker. Physical GDB session 9296813e-2def-4ab8-8491-b476d5aba1a0 passed backtrace, stepi and continue to app_main without firmware replacement. Its disconnect exposed the SDK transport not forwarding stdin EOF; termination left conservative probe and TCP leases. After exact owner 19616, GDB, OpenOCD and Python supervisors were absent and TCP 3333 unoccupied, the two records were preserved in the account lease root's operator-recovery-20260925-19616 archive. No unrelated lease or runtime safeguard changed.

The shutdown coordinator now starts registered cleanup on stdin EOF. A real child-process regression test verifies asynchronous cleanup completes before exit (4 shutdown tests passed). USB debugger custody now also holds the matching native serial identity and canonical endpoint, with fresh identity/port checks before spawn (15 focused discovery/custody/startup tests passed). Physical session 118f20c9-ff7a-4fd8-8e67-d6db9b77613a started successfully; the legacy upload custody path rejected COM7 before any child spawn with DEVICE_HANDOFF_PENDING. Client disconnect then released every lease from that session automatically. No manual recovery was needed for the fixed disconnect.

Fresh agent_resolve_target exposed that raw INI parsing ignored the existing native fixture's inherited board. Advanced configuration now uses PlatformIO's computed metadata rather than guessing inheritance. Seven focused target-resolution tests and TypeScript passed. Actual binding resolved hil-esp32s3-native to esp32-s3-devkitc-1 and the same USB serial. Native flash/verify and custom partition acceptance remain in progress; prior UART, core-dump and unchanged dashboard evidence is retained, not relabeled final-revision evidence.


### 2026-09-25 � Native flash/verify and custom layout acceptance completed

The first native upload succeeded with verified image hashes and 30 healthy lines, but verification correctly timed out because the fixture printed indefinitely and the required quiet window never occurred. Only the native environment now defines PIO_HIL_HEARTBEAT_UNTIL_MS=15000; UART behavior and serial command/crash handling remain unchanged. This observed failure justified the affected fixture rebuild/upload. No quiet-window or crash check was weakened.

The corrected flash_verification returned ok=true/verdict=pass after 25.427 seconds: 15 fresh healthy lines, the configured ten-second quiet window, no runtime failures or lost/truncated evidence, cleanupPending=false. Upload logged matching board MAC 14:c1:9f:dc:20:4c and verified flash hashes. ELF SHA-256 29f15c4809965d38772b96cda405c8e4788d7f938dddd749dd4cdc875b69b0f8; firmware 99327f62da7421140192de4e3ffcb4dd7140e9d54cdb319bd41fd717a58e73b3; retained manifest 2af9f7ab39245948c31aa2d629ff072b262427d64fffa5ee06b14e78a012d399; firmware_identity=embedded_hash_match. Account command log: target-528ec4b1-7e53-45d1-b03f-0ef09047166a.log. Fixture source line changes do not relabel historical crash/ELF evidence as this firmware revision.

The subsequent authorized partition_table read at 0x8000 returned six physical device entries and device.diff=[] against partitions-hil.csv, zero layout errors/warnings, firmware fitting both application slots. Read sector SHA-256 b26bcdf85613532c35861806dc3bdfabbbd105b7fd7d22d52f39fc1701ca4f6f; CSV SHA-256 cf6e9809bc004a7eb6cae9a71b89dcf75be502b629a82c545c58c60f78e57588. Evidence includes the device result, not only the top-level offline-layout summary. Native partition log: .pio-mcp-workspace/logs/debug/debug-2026-09-25T16-10-28-085Z-c6a602d9.log under the fixture.

EOF handling is explicitly installed only for MCP stdio, preserving standalone dashboard/CLI lifetime. The shared shutdown action is unchanged from the successful physical disconnect; the scoped hook's real-process regression and TypeScript passed. In total, 26 focused tests cover changed custody/discovery/startup, EOF cleanup and inherited binding. Changed-source lint passed with only existing index.ts warnings. Plugin runtime and 36-file inventory were assembled together from the updated source and unchanged tested dashboard assets; eight-skill runtime validation passed. These local results are not a claim that the original full acceptance or publication gates passed.


### 2026-09-25 � Required CI and publisher setup

Commit 22a9d73b6883847f5eab6528778f4e34ce5be08d passed required CI 36159406829 and installer configuration acceptance 36159406866. These results apply to that commit; no physical or minimum-OS tests are inferred from CI.

GitHub device-flow refresh completed on this host; the account package endpoint now returns an empty container inventory instead of the former read:packages 403. GET user identifies jl-codes/User and repository permissions include admin. GitHub's Container registry documentation confirms personal-account namespaces and repository GITHUB_TOKEN publication (https://docs.github.com/en/packages/working-with-a-github-packages-registry/working-with-the-container-registry). Existing release.yml grants packages:write only within the protected ghcr job. Automatic review initially rejected promoting all ten targets; after the user explicitly approved those exact ten targets and ownership/rules were checked, the configuration operation succeeded. The marker was read back and matches the seven required names plus flash-agent, flash.agent and flash_agent. This is configured publication authority, not published packages or a promise of registry acceptance.

PyPI grant submission reached password reconfirmation. The user's three grant approvals remain valid; no repeat approval is needed after the user completes the existing password form. Native OTA source preparation is separate from the completed native USB acceptance; it contains no credentials and is pending private input/build/live verification. Wi-Fi profile inspection exposed only authentication type and key presence, never the saved key.

### 2026-09-25 � Completed same-board OTA and restored native fixture

Fresh USB serial bound VID:PID 303A:1001, serial/MAC 14:C1:9F:DC:20:4C to 192.168.1.144:3232. Authenticated firmware OTA completed with exit 0/progress 100, image SHA-256 2dfc609366ca382f4bd85aff642336763bae95e43e8f0a8a74f05250b9f9f973 (784256 bytes), embedded ELF hash 828acec0267bd3dfbb5899b7baaf3b6db13bb5b60719dcf533289cbd1d782280. Independent serial observation changed from ota-base-20260925 to ota-updated-20260925 with the same MAC/IP. ELF correspondence was identity_unverified because the filesystem build removed the live ELF; no full ELF correspondence is claimed. The exact retained firmware was reused with its hash checked, not rebuilt. Success log: target-9631c51e-03af-40be-8c51-5015499ac6d8.log.

Filesystem OTA completed with exit 0/progress 100, image SHA-256 9f982e4c1de1d4cac097ab2a31ce5dfa87f716febab570a41e4d321d94938b6e (1703936 bytes), log target-84459a6b-a3f0-40a5-8a20-70d692793080.log. A new same-board serial session returned both the updated firmware identity and PIO_HIL_FS:PIO_HIL_FILESYSTEM_20260925. The prior authenticated no_callback result is bounded failed-transfer evidence, not a passing transfer or corruption/recovery test.

Before the scoped firewall helper mutated anything, the existing Python TCP rule had changed externally from Block to Allow. The exact-identity guard refused to modify it; no temporary rule was created, and no rule was disabled by this task. The subsequent transfer passed using that external state. Final read confirmed the original rule's then-current Enabled/Allow state was untouched and the task-specific temporary rule was absent. Do not claim the task restored a Block state it never changed.

Restoration task bfec080b-a20e-430f-9224-0c298aa4126e uploaded the credential-free hil-esp32s3-native fixture successfully with flash hash verification. Rebuild was necessary because Core filesystem preparation had removed its previous output. Restored firmware SHA-256 e937a74c4de7ac6065345babf4cd5dae6e7764c3e60bed80da88988000ed7f6e; ELF 5faa6148617a1730d4ce9a89ffab6e975fa30343009d1180c999a53f70627533. A bounded serial OTA_ID command now returned PIO_HIL_ECHO:OTA_ID, establishing the native command handler rather than the OTA fixture handler. This is restoration evidence, not a rerun of the earlier crash/quiet-window acceptance.

Original physical acceptance step 4/PAR-HW-04 also required the legacy filesystem path. Task bc2554fa-c70c-49e8-aa69-81a265b9754f completed upload_filesystem on the same native environment: 1703936 bytes written at 0x650000, hashes verified, exit success in 13.803 seconds. Native application firmware was not replaced by this filesystem operation. The existing dashboard visibly retained historical upload/monitor task records and showed current active port COM7; this observation is not a new claim that an old dashboard process loaded the current source.

All owned serial sessions were closed and the MCP acceptance client exited successfully. Removed private config.h/auth.json and the exact local/retained credential-bearing OTA image copies after verifying their identity; no credential was printed, committed or selected for release artifacts. Existing Windows Wi-Fi profile was not modified. Required CI and installer checks on 8f612ce3 passed; no duplicate workflow or discretionary suite was dispatched.

### Pinned 40-tool evidence reconciliation (8f612ce3)

All 40 pinned IDs are accounted for exactly once below. Named tests assert behavior, not just tool registration, and passed in existing relevant CI; several deliberately mock hardware/process boundaries. The dated real execution evidence elsewhere in this same record supplies the indicated live scope. This mapping is not a fabricated full acceptance pass or a final same-revision acceptance packet. Historical outcomes retain their original revision/ELF identities and applicability limits.

| Pinned requirements | Capability / concrete evidence | Remaining limit |
| --- | --- | --- |
| PAR-01, PAR-13 | Board lookup/filtering; tests/board-compat.test.ts | Filter/order/error assertions; actual Core board discovery and current ESP32-S3 environment selection are recorded. |
| PAR-02, PAR-03, PAR-04, PAR-35, PAR-38, PAR-39 | Build/check/clean/targets/tests/upload; tests/clean-compat.test.ts, tests/check-report.test.ts, tests/run-target.test.ts, tests/test-compat.test.ts, tests/test-report-execution.test.ts | Command arguments, timeout/failure/report and policy assertions; actual fixture builds and USB firmware/filesystem uploads. This does not turn mocked test-run cases into physical target-test evidence. |
| PAR-05, PAR-10, PAR-23, PAR-36 | Core dump/backtrace/partitions/size; tests/coredump-compat.test.ts, tests/decode-compat.test.ts, tests/partition-compat.test.ts, tests/analysis-handlers.test.ts | ELF/provenance and error assertions plus real Xtensa tools, physical ESP32 core acquisition/decoding and custom device partition comparison. Cortex-M physical fault capture remains PAR-HW-02. |
| PAR-06, PAR-07, PAR-08, PAR-09 | Debugger session lifecycle; tests/debug-session-compat.test.ts, tests/debug-local-startup.test.ts, tests/debug-command.test.ts, tests/debug-probe-custody.test.ts | MI transcript/session ownership and cleanup assertions plus native JTAG start/backtrace/step/continue, upload exclusion and disconnect cleanup. Resolved alternative backend mechanisms are not reopened. |
| PAR-11 | Dependency inspection; tests/dependency-inspection.test.ts, tests/dependency-audit.test.ts | Manifest/graph/status assertions and recorded populated real Windows MCP dependency inspection. |
| PAR-12 | Flash/verify; tests/flash-verification-compat.test.ts | Workflow error projection with physical native upload hashes, fresh healthy lines and ten-second stability window. |
| PAR-14, PAR-16, PAR-17, PAR-18, PAR-19, PAR-20, PAR-21, PAR-22, PAR-30 | Devices/serial/memory/port diagnosis; tests/device-compat.test.ts, tests/memory-compat.test.ts, tests/memory-capture.test.ts | Owned capture/cursor/redaction/telemetry assertions and recorded physical echo/crash/recovery/exclusion/reopen. Memory parser fixtures are not a physical meter result. |
| PAR-24, PAR-25, PAR-26, PAR-27, PAR-28, PAR-29 | Package commands; tests/package-compat.test.ts, tests/package-compat-mcp.test.ts | Pinned request/result/permission cases and recorded real modern package command acceptance; no new package mutation was performed for this reconciliation. |
| PAR-31 | Power profiling; tests/power-compat-policy.test.ts, tests/power-meter-client.test.ts, tests/power-triggered-meter.test.ts | Software protocol/permission/trigger and cleanup coverage only; physical PPK2 measurement/source safety remains explicitly unpassed. |
| PAR-15, PAR-32, PAR-33, PAR-34, PAR-37 | Project/environment/metadata/system; tests/project-compat.test.ts, tests/init-compat.test.ts, tests/system-compat.test.ts | Argument/result/configuration assertions, real Core metadata and inherited-environment binding. Existing project initialization evidence is reused only for unchanged behavior. |
| PAR-40 | OTA; tests/ota-upload.test.ts, tests/ota-transfer.test.ts, tests/espota-process.test.ts | Software failure/custody/protocol coverage plus same-board ESP32 authentication, bounded no_callback failure, successful firmware/filesystem transfers and independent serial verification. ESP8266 remains unpassed. |

Physical MCP transcript after client shutdown: .platformio-mcp/serial-acceptance-session.jsonl, SHA-256 2c3a2d3d98ffcf04b4a1ca16593d4c73e7e65da6c340b4f24379558357a30ca0. This is retained local evidence, not an uploaded release artifact or complete aggregate acceptance packet.


### 2026-09-25 � Maintainer amendment and retained host acceptance

The authoritative 3.1.0 amendment is recorded once in the acceptance catalog's releaseScope. Validators preserve the complete inventory, report the five named certifications as deferred (not passed), and still reject missing ESP32 evidence, invented waivers and fabricated Cortex-M passes. Six focused collector/validator tests passed. PPK2 protocol still requires an explicit ampere/source mode, voltage and current limit; independent sourcing authorization is unchanged.

Actual isolated Codex 0.155.0-alpha.16.4 host testing exposed a plugin launch defect: relative arguments were evaluated from the workspace because .mcp.json omitted cwd. Added plugin-root cwd and the existing opt-in compatibility/policy/data/dashboard environment selectors. In new isolated standalone and installed-plugin homes, 110 enabled tools were visible and both build_project and pio_build were absent; direct requests for each received the host's disabled-tool error before execution. The final plugin run used the shipped declaration without cache edits. No model, physical operation or user configuration was used. Five focused plugin manifest checks passed. OpenAI's plugin_config.rs explicitly resolves a relative cwd against the plugin root. Host observations retain the original checkout identity and declaration hash rather than claiming a clean future revision.

The retained real Cortex-M ELF software check was absent from earlier evidence (distinct from deferred physical Cortex-M certification). A build-only nucleo_f401re/CMSIS fixture pinned to ststm32 19.4.0 compiled successfully through MCP. Real GNU nm/size/addr2line verified source resolution, unknown frames, expected/wrong hashes, symbol ranking/filtering and totals; no Cortex-M device was accessed. Exact observed hashes are in analysis-cortex-m-windows-evidence.json. The original native PlatformIO passing/failing test and static-analysis report requirement now executes on the available Linux CI runner; its result is pending, not inferred from mocked report tests.

Publication jobs now isolate npm from PyPI/GHCR: an npm registry/authentication failure no longer fails the shared artifact build and suppresses independent publishers. Artifact validation, provenance and protected environments remain required. Workflow YAML and dependency separation checks passed. Merge/publication and the retained aggregate evidence gate remain pending.
