...
Existing goal runtime reports active as verified on 2026-09-25; objective text was not edited by the agent. Branch: `codex/platformio-parity-permissions`.
...
1. **Software parity — Codex:** Debugger commits through fa99fa08 are pushed to PR #26. Standalone remote startup uses operator-owned target maps through the existing supervised path; numeric and now bounded, pinned DNS selections are implemented. Standalone st-util and explicitly configured custom pipe commands remain unsupported. Core's automatic OpenOCD pipe choice is covered by the owned loopback TCP path; the missing default readiness pattern is now fixed. real remote debug acceptance remains required. Complete the pinned 40-tool behavioral/result/error contracts and legacy retention requirements; registration and CI alone do not satisfy them.
2. **Required acceptance — Codex plus hardware owner:** reuse existing native installs, container builds, POSIX signal results and POL-05 evidence where unchanged. Windows installed-launcher Ctrl-C now passes on the reused artifact with matching launcher source; complete remaining minimum-platform and actual host enforcement evidence, and remaining physical acceptance for live flash/verify stability, Cortex-M crashes, USB disconnect/reconnect, debug-probe cleanup, partitions/core dumps, ESP32/ESP8266 firmware/filesystem OTA, and PPK2 measurement/source limits/output-off. ESP32-S3 echo/crash decoding, cross-process serial exclusion and reopen-after-cleanup now have the bounded evidence below. Exact host/project/environment/device bindings requested from user; unavailable hardware remains a blocker, not a passing result.
