...
Each request accepts at most 4,096 pattern characters, 10,000 lines and 1 MiB of line data. Regex workers have a separate 5-second startup deadline and a 1-second default compilation/execution deadline (configurable internally from 1 to 2,000 ms), memory limits, and a maximum of four concurrent workers. Excess concurrent work returns PATTERN_BUSY. Timeout, unsupported syntax and input limits are explicit errors; no partial match result is treated as success. Workers are terminated before completion is reported, including errors.
...
Waits are bounded to 120 seconds, with at most 32 pending readers. Cancellation affects only that reader. Disconnect, stop and error retain unread text and terminal state. Literal matching remains the default; regex reads use the bounded matcher above. Its separate startup and execution deadlines may extend the read deadline, and matcher errors are not reported as successful reads. A full page returns immediately even when the requested pattern has not matched.

This module is connected to the internal direct-session service, but not yet exposed through a public tool. Session authorization, physical-device ownership, writes, cleanup, compatibility adapters and real-device acceptance remain required before serial-session parity can be claimed. Existing monitoring behavior is unchanged.

The worker signals readiness before compilation or matching starts. The parent starts the execution timer before sending permission to run. Slow host startup therefore does not consume the regex budget; startup failure returns PATTERN_WORKER_STARTUP_TIMEOUT, while slow compilation/matching returns PATTERN_TIMEOUT. Both paths terminate the worker and retain the concurrency slot until termination completes.
