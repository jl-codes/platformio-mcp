...
Inspection-approval policies can pass a separate `discoveryApprovalId` in trusted request context. This is consumed by one `list_devices` call, bound by the existing approval store to its canonical workspace and arguments. The ordinary session `approvalId` is never forwarded to discovery. Repeated startup verification under an inspection-approval policy still needs a bounded composite authorization adapter; replaying this one-use ID for each enumeration is not supported.

## Bounded startup discovery authorization

`PolicySerialSessionService.startWithDiscovery` now authorizes internal `serial_startup_discovery`, mapped to the existing `list_devices` category, with the exact startup request and a four-snapshot limit. The separate discovery approval is consumed once. An internal asynchronous context then permits only that startup's initial snapshot and three revalidations, under the same canonical workspace and revision guard, for at most 30 seconds. The context expires on success or failure and cannot be reused by a later startup. Direct `list_devices` grants and startup discovery grants remain distinct concrete operations.
...
## Redaction integrated into direct sessions

Direct session buffers now enable known-format secret filtering. An 80-character rolling marker scanner observes decoded input before line truncation, tracks PEM blocks across chunks and ring eviction, and conservatively suppresses an entire PEM-bearing line. It retains no unbounded block body. Completed lines are stored filtered; partial previews are filtered without advancing framing state. Ordinary incomplete-line text remains bounded in memory until line completion. Matching uses filtered snapshots, so a secret value is not exposed through match results.

Replacement text is clipped at whole UTF-8 code points when needed to preserve line/storage/response limits. Additive redactionApplied and redactionOutputMayBeTruncated fields distinguish filtering and replacement clipping from source-byte truncation counters. The underlying buffer's default remains unfiltered for internal callers that explicitly manage policy; direct sessions enable filtering unconditionally. This is detection of known textual formats, not a guarantee that all unknown secrets are recognized. Existing legacy monitor output is unchanged by this integration.

## Packaged native backend

The plugin builder now creates a separate Node 20 CommonJS serial backend bundle plus the pinned bindings-cpp prebuilds for Windows x64, universal macOS x64/arm64, and Linux x64/arm64 (glibc and included upstream musl variants). Backend dependencies carry copied licenses and a version/license manifest; all files are included in the plugin checksum inventory. No install scripts run as part of plugin startup.

The shared loader uses the packaged backend beside the main bundle when present and the normal pinned npm dependency for ordinary source/npm execution. A broken packaged backend fails explicitly rather than silently substituting another installation. An isolated Windows test bundles the actual loader and loads native exports without node_modules, enumeration or opening a port. Other host loading, wheel packaging and physical acceptance still require evidence. Public tools remain unwired.
