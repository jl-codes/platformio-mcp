...
export function namespaceCoverage(inventory, observations, githubPublication) {
  ...
  const channels = ["npm", "pypi", "ghcr", "mcp", "plugin", "github", ...(inventory.unsupportedChannels ?? [])];
  ...
  for (const alias of requested) for (const channel of channels) {
    const candidates = inventory.entries.filter(entry => entry.registry === channel && (
      ...
      (channel === "ghcr" || channel === "mcp" || channel === "github") && entry.name.endsWith("/" + alias)
    ));
    const entries = candidates.map(entry => {
      const observation = observed.get(`${channel}:${entry.name}`);
      const landing = channel === "github" && entry.role === "official_landing" && entry.publicationControlVerified === true
        ? githubPublication?.repositories?.find(row => row.name === entry.name && row.url === `https://github.com/${entry.name}` && row.canonicalSource === inventory.canonicalSource && row.private === false && Number.isSafeInteger(row.repositoryId) && row.repositoryId > 0 && /^[a-f0-9]{40}$/.test(row.readmeBlobSha ?? "") && Number.isFinite(Date.parse(githubPublication.observedAt)))
        : undefined;
      return {
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  const result = namespaceCoverage(read("distribution/namespaces.json"), read("docs/reviews/platformio-namespace-observations.json"), read("docs/reviews/github-alias-publication.json"));
