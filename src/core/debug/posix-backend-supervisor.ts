...
import json, os, select, signal, subprocess, sys, threading, time
...
try:
    ...
    if guardian == 0:
        ...
        def notify(**fields):
            os.write(write_fd, json.dumps(fields).encode("ascii") + b"\n")
        try:
            os.setsid()
            ...
            child = subprocess.Popen([executable] + args, cwd=cwd, stdin=subprocess.DEVNULL,
