...
const describeMcp = process.env.RUN_MCP_E2E === "1" ? describe : describe.skip;
...
async function parseToolPayloadFromText(text: string): Promise<unknown> {
  ...
  if (markerIdx === -1) {
    throw new Error(
      `Could not resolve spooled payload path from message: ${text}`,
    );
  }
  ...
  if (pathEnd === -1) {
    throw new Error(
      `Could not resolve spooled payload terminator from message: ${text}`,
    );
  }
...
describeMcp("PlatformIO MCP Server E2E Integration", () => {
  ...
  beforeAll(async () => {
    ...
    // Without a policy file the workspace falls back to `flash_requires_approval`,
    // which correctly gates upload_firmware, upload_filesystem, install_library
    ...
    console.log(
      `Starting MCP server integration test on tmp folder: ${tempProjectDir}`,
    );
  ...
  it("should list available boards", async () => {
    ...
    const parsed = (await parseToolPayloadFromText(
      result.content[0].text,
    )) as Array<{ id: string; name: string }>;
    expect(Array.isArray(parsed)).toBe(true);
    const hasUnoName = parsed.some((board) =>
      board.name.includes("Arduino Uno"),
    );
  ...
  it("should initialize a new project", async () => {
    ...
    const iniContent = await fs.readFile(
  ...
  it("should gracefully handle upload bounds when no serial port", async () => {
    ...
    // The server reports failures as a structured payload, not a message
    // containing the word "Error", so assert the contract: it failed, and it
    // said why. That holds whether or not isError is set.
    const parsed = JSON.parse(result.content[0].text);
  ...
  it("should install a library explicitly into the test workspace", async () => {
    ...
    // "bblanchon/ArduinoJson" is PlatformIO's canonical identifier form. This
    // used to fail validation before a slash was permitted, and the old
    // assertion hid that by accepting any error whose text said "Error".
    const parsed = JSON.parse(result.content[0].text);
    expect(parsed.success).toBe(true);
  }, 60000);
  ...
  it("still gates a hardware write behind approval under the default profile", async () => {
    ...
    try {
      ...
      const result = (await harness.client.callTool({
        name: "upload_firmware",
        arguments: { projectDir: gatedDir, port: "/dev/ttyNONEXISTENT" },
      })) as { content: Array<{ type: string; text: string }> };

      const parsed = JSON.parse(result.content[0].text);
      expect(parsed.success).toBe(false);
      expect(parsed.summary).toContain("approval");
      expect(parsed.data?.policyDecision?.status).toBe("requires_approval");
    } finally {
      await fs.rm(gatedDir, { recursive: true, force: true });
    }
  }, 30000);
